document.addEventListener('DOMContentLoaded', async (event) => {
        ...
        if (editContainer) {
          ...
          if (editButton && modal) {
            ...
            editButton.addEventListener('click', () => {
              ...
              showImagesInModal(allJobs); // Agregamos esto para cargar las imágenes en la modal cada vez que se hace clic en el botón de editar
  ...
  function showImagesInModal(jobs) {
    const modalImagesDiv = document.getElementById('fotomodal');
  
    modalImagesDiv.innerHTML = ''; // Limpiar las imágenes anteriores.
  
    // Para cada trabajo, crear un nuevo elemento figure y añadirlo a modalImagesDiv.
    jobs.forEach(job => {
      const figure = document.createElement('figure');
      figure.setAttribute('data-photo-id', job.id); // Añadir atributo de datos para identificar la foto
...
if (submitButton) {
  submitButton.addEventListener('click', async (event) => {
    ...
    const confirmation = confirm('¿Estás seguro de que quieres eliminar todos los projets?');
    if (confirmation) {
      try {
        const authToken = localStorage.getItem('authToken');
        ...
        if (response.ok) {
          console.log('Se eliminaron todos los projets correctamente');
          // Eliminar los elementos de la galería
          const galleryContainer = document.querySelector('.gallery');
          galleryContainer.innerHTML = '';
        } else {
          throw new Error('Error al eliminar los projets. Por favor, inténtalo de nuevo.');
        }
      } catch (error) {
        console.error('Error al enviar la solicitud DELETE:', error);
      }
